...
- Highchars。js 国外的前端数据可视化库，非商用免费，被许多国外大公司所使用
- AntV 蚂蚁金服全新一代数据可视化解决方案

<strong> 通俗理解： Hightcharts 和 Echarts 就像 office 和 WPS 的关系 </strong>

#### 小洁

- 数据可视化主要目的：借助于图形化手段，清晰有效地传达与沟通信息
- 数据可视化在我们互联网公司中经常用于通用数据报表，移动端图表，大屏可视化，图编辑等
- 数据可视化库有很多，接下来我们重点学习 Echarts
...
<strong> 市场需求：</strong>
应对现在数据可视化的趋势，越来越多企业需要在很多场景（营销数据，生产数据，用户数据）下使用，可视化图表来展示体现数据，让数据更加直观，数据特点更加突出。<br />

<strong> 学习阶段需求：</strong>
项目对我们同学来说，起着承上启下作用。

- 呈上：
  复习以前学习内容 <br />
  html5 + css3 布局相关技术 <br />
  javascript/jQuery 相关技术 <br />

- 启下
  为学习服务器编程做铺垫 <br />
  如何把服务器里面的数据渲染到页面中 <br />
...
ECharts 是一个 javascript 实现的开源可视化库，可以流畅的运行在 PC 和移动设备上，兼容当前绝大部分浏览器，（IE89/10/11，chrome，Firefox，Safari 等），底层依赖矢量图形库，ZRender，提供直观，交互丰富，可提高个性化定制的数据可视化图表。<br />

- 开源免费，功能丰富，社区活跃

<a href="https://echarts.apache.org/zh/index.html" target="_bloack"> 官网地址 </a>
...
:::tip
ECharts 能满足绝大多数可视化图表实现，使用方便，功能强大，是实现数据可视化的最优选择之一
:::
...
:::warning 注意
这里只要求记住 ECharts 使用的步骤，具体修改定制。
:::

- 步骤 1：下载并引用 echart.js 文件 ------> 图表依赖这个 JS 库
- 步骤 2：准备一个具备大小的 DOM 容器 ------> 生成的图表会放入这个容器内
- 步骤 3：初始化 rcharts 实例对象 ------> 实例化 echarts 对象
- 步骤 4：指定配置项和数据（option）------> 根据具体需求修改配置选项
- 步骤 5：将配置项设置给 echarts 实例对象 ------> 让 echarts 对象根据修改好的配置生效
...
<script>
  ...
  // 将配置项和数据（option）设置给实例对象 没有Chart
...
- grid：直角坐标系内绘图网络
  ...
  - containLabel: false<br />
    当刻度标签溢出的时候，grid 区域是否包含坐标轴的刻度标签，如果为 true，则显示，false 不显示
...
:::warning 注意
先了解这 9 个配置的作用，其余配置还有具体细节我们查阅文档，文档菜单--配置项手册，学 echarts 关键在于学会查阅文档，根据需求修改配置。
:::

- series：系列列表 <br />
  type：类型（什么类型的图表）比如 line 是折线，bar 柱形等<br />
  name：列表名称，用于 tooltip 的显示，legend 的图例筛选 变化 <br />
  stack：数据堆叠。如果设置相同值，则会数据堆叠 <br />
  数据堆叠：第一个数据值= 第一个数据值 + 第二个数据值 <br />
  第三个数据值= 第二个数据值 + 第三个数据值...依次叠加 <br />
  如果给 stack 指定不同值或者去掉这个属性则不会发生数据堆叠 <br />
...
- 设计稿是 1920px
- PC 端适配：宽度在 1024~1920 之间页面元素宽度自适应

#### 适配方案

- 媒体查询
- js 自动获取
- flexible.js 检测浏览器宽度 修改 html 文字大小

适配最终方案：flexible.js + rem 单位 + flex 布局 <br />

- PC 端
  flexible.js 把屏幕分成 24 等分。<br />
  PC 端的效果图是 1920px <br />
  cssrem 插件的基准值是 80px , 插件配置按钮--配置扩展设置--Root Font Size 里面设置，需要重启<br />
  rem 值自动生成 <br />
  要把屏幕宽度约束在 1024~1920 之间有适配，用媒体查询。 <br />

### 数据可视化项目开发

项目核心：先布局，再利用 ECharts 填充图表 <br />
...
盒子大小不一。但是边框样式相同，此时就需要边框图片来完成 <br />
为了实现丰富多彩的边框效果，在 CSS 中，新增了 border-image 属性，这个新属性允许指定一幅图片作为元素的边框。 <br />
...
把四个角切出去，（九宫格的由来），中间部分可以铺排、拉伸或者环绕。 <br />
